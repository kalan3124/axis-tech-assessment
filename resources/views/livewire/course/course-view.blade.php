<div x-data="{ openModal: false }">
    <div class="container mx-auto px-4 space-y-6">
            ...
            <div class="relative p-4 w-full max-w-2xl h-full md:h-auto">
                ...
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div class="flex justify-between items-start p-4 rounded-t border-b dark:border-gray-600">
                        ...
                        <button type="button" id="course-modal-close-btn"
                    ...
                    <div class="p-6 space-y-6">
                        @livewire('course.course-create', key('create-course'))
                    </div>
                </div>
            </div>
        </div>
        ...
        <div id="courseUserModal" tabindex="-1" aria-hidden="true"
            class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 w-full md:inset-0 h-modal md:h-full">
            <div class="relative p-4 w-full max-w-2xl h-full md:h-auto">
                <!-- Modal content -->
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                            ...
                            <tbody>
                                ...
                                @else
                                    <tr>
                                        <td colspan="2" class="py-5">
                                            <div class="flex justify-center">
                                                <label class="text-sm" for="">There are no any assigend
                                                    users</label>
                                            </div>
                                        </td>
                                    </tr>
        ...
        <div id="popup-course-modal" tabindex="-1" aria-hidden="true"
            class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 w-full md:inset-0 h-modal md:h-full">
                ...
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div class="p-6 text-center">
                        <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">Are you sure you want to
                            delete?</h3><br>
                        <button wire:click="deleteCourse" type="button"
                    ...
                    @if (count($courses) > 0)
                        @foreach ($courses as $course)
                            <tr
                                ...
                                <td class="px-6 py-4">
                                    <button type="button" wire:click="getCourseUsers({{ $course->id }})"
                                        class="text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">
                                ...
                                <td class="px-6 py-4">
                                    <button type="button" wire:click="getPartUsers({{ $course->filterUsers }})"
                                        class="text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">
                                        <svg class="w-6 h-6" fill="none" stroke="currentColor"
                                            viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                <td class="px-6 py-4">
                                    <div class="inline-flex rounded-md shadow-sm" role="group">
                                        <button type="button" wire:click.prevent="editCourse({{ $course->id }})"
                                        ...
                                        <button type="button" wire:click="setDeleteId({{ $course->id }})"
                                            class="text-white bg-red-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">
                                            <svg class="w-6 h-6" fill="none" stroke="currentColor"
                                                viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    @else
                        <tr>
                            <td colspan="4" class="py-5">
                                <div class="flex justify-center">
                                    <label class="text-sm" for="">There are no any courses to show</label>
                                </div>
                            </td>
                        </tr>
                    @endif
...
@push('scripts')
    <script>
        document.addEventListener('course-added', function() {
            $('#course-modal-close-btn').click();
        });
        document.addEventListener('show-edit-modal', function() {
            const targetEl = document.getElementById('updateCourseModal');
            const modal = new Modal(targetEl);
            modal.toggle();
        });
        document.addEventListener('show-users-course-modal', function() {
            const targetEl = document.getElementById('userCourseModal');
            const modal = new Modal(targetEl);
            modal.toggle();
        });
        document.addEventListener('show-course-users-modal', function() {
            const targetEl = document.getElementById('courseUserModal');
            const modal = new Modal(targetEl);
            modal.toggle();
        });
        document.addEventListener('pop-dialog', function() {
            const targetEl = document.getElementById('popup-course-modal');
            const modal = new Modal(targetEl);
            modal.toggle();
        });

        document.addEventListener('pop-close-dialog', function() {
            closeModal();
        });
